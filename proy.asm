...
mover_izq:				;Mover a la izquierda
...
MOV_BOLA proc
	...
	colision_bricks: ;verifica si hay colisión con bloques
		...
		confirmar_colision:		;Confirma si colisiono con un bloque
		...
		jne colision_player ;si no es igual, checa si hay colisición con player
		...
		azul:
			dec [temp]
			posiciona_cursor [ren_aux],[col_aux]
			imprime_caracter_color 219d,02h,bgNegro
			inc [col_aux]
			cmp [temp],0
			jne azul
			;score
	...
	;primero comprobamos si se encuentra en el renglon de la barra
	cmp ah, [bola_ren]
	jne lim_izq

	;comprobamos si se encuentra dentro del rango de columnas que ocupa el jugador
	sub al, 2d
	cmp [bola_col], al
	jb lim_izq
	add al, 4d
	...
	lim_super:
		cmp [bola_ren], 2d ;[renbola] 5, 4 ,3 <= 6
		jae lim_infer
		mov [dir_ver], 1d
		jmp mov_bola_salida

	lim_infer:
		cmp [bola_ren], 22d ;limite
		jbe mov_bola_salida
		mov [dir_ver], -1d
		;call BOLA_ORIGEN
